...
# Karten erstellen zu early greenup und late maturity
library(tmap)
library(terra)

tmap_mode("plot")  # statische Karte; für interaktive Karte: "view"

# Raster vorbereiten
early_r <- early_greenup
late_r  <- late_maturity

# Funktion für Layer, falls leer
...
tm <- tm_tiles("OpenStreetMap") +
  tm_shape(mask_forest) +
  tm_raster(palette = "lightgreen", alpha = 0.3, title = "Forest Mask")

tm_early <- safe_tm_shape(early_r, "yellow", "Early Greenup")
tm_late  <- safe_tm_shape(late_r, "red", "Late Maturity")

# Layer nur hinzufügen, wenn vorhanden
if(!is.null(tm_early)) tm <- tm + tm_early
if(!is.null(tm_late))  tm <- tm + tm_late

# Layout & Titel
tm <- tm + tm_layout(legend.outside = TRUE) +
