---
title: "Report Geoprocessing Module 3"
output:
  html_document: default
  pdf_document: default
authors: "Laurent Voser & Moritz Weber"
---
...
```
# Introduction 
The goal of this exercise was to go through the R book "A Handful of Pixels" and solve exercises 8.1.1 and 8.1.2. The main subject of these exercises was phenology modelling. 
...
#Kontrolle Plot
png("../plots/phenology_patterns_2010.png", width = 1200, height = 600, res = 150)
par(mfrow = c(1, 2))
terra::plot(early_greenup, main = "Early Greenup 2010")
terra::plot(late_maturity, main = "Late Maturity 2010")
invisible(dev.off())

knitr::include_graphics("../plots/phenology_patterns_2010.png")
```

### Boxplot: Greenup und Maturity vs. Elevation
```{r}
dem_us <- terra::rast("../data-raw/USA_elv_msk.tif")
...
# Convert SpatRasters to a format leaflet understands
# leaflet needs WGS84 (EPSG:4326)
forest_wgs  <- project(ifel(as.numeric(mask_forest_aligned) == 1, 1, NA), "EPSG:4326")
early_wgs   <- project(ifel(as.numeric(early_greenup) == 1, 1, NA),        "EPSG:4326")
late_wgs    <- project(ifel(as.numeric(late_maturity)  == 1, 1, NA),        "EPSG:4326")

# Colour palettes (single value rasters: just one colour)
pal_forest <- colorNumeric("Greens",  values(forest_wgs), na.color = "transparent")
pal_early  <- colorNumeric("Purples", values(early_wgs),  na.color = "transparent")
pal_late   <- colorNumeric("Reds",    values(late_wgs),   na.color = "transparent")

leaflet() %>%
  addProviderTiles(providers$OpenStreetMap, group = "OSM") %>%
  addRasterImage(forest_wgs, colors = pal_forest, opacity = 0.7, group = "Forest Mask") %>%
  addRasterImage(early_wgs,  colors = pal_early,  opacity = 0.6, group = "Early Greenup") %>%
  addRasterImage(late_wgs,   colors = pal_late,   opacity = 0.6, group = "Late Maturity") %>%
  addLayersControl(
    baseGroups    = "OSM",
    overlayGroups = c("Forest Mask", "Early Greenup", "Late Maturity"),
    options       = layersControlOptions(collapsed = FALSE)
  ) %>%
  addLegend(pal = pal_early, values = values(early_greenup),  title = "Early Greenup", position = "bottomright") %>%
  addLegend(pal = pal_late,  values = values(late_maturity),   title = "Late Maturity",  position = "bottomright")
```


## Discussion
The analysis reveals widespread early greenup across the Adirondack forested area in 2010, suggesting an unusually warm spring relative to the 2001–2009 baseline. This is likely linked to large-scale atmospheric patterns such as the North Atlantic Oscillation (NAO), which strongly influences spring temperatures in the Northeastern United States. Late maturity anomalies are spatially more limited and regionally clustered, possibly driven by localised late-season frost events or cloud cover anomalies delaying canopy development despite the early seasonal start.

Regarding elevation, unaffected areas tend to occur at higher elevations. This is somewhat counterintuitive, but can be explained by the warming signal in 2010 being most pronounced at lower elevations, while higher elevation sites remained cold enough to keep phenology within the long-term norm. Late maturity areas are situated slightly lower than early greenup areas, suggesting that factors beyond temperature – such as soil moisture or local wind exposure – also play a role.

It should be noted that the MODIS 500m resolution limits detection of fine-scale patterns, the nine-year reference period may not fully capture natural variability, and the ±1 SD threshold produces a binary classification that simplifies the continuous nature of phenological change.
